...
class Controller(object):
    ...
    def __init__(self):
        ...
        self.__pc_roster = {}
        self.__skill_shorts = {}
        pc_filepaths = ['preferences/Rasmus_Shawn Everette Slow Curve Manning.xml']
        ...
        for weapon in weapons:
            ...
            wpn_table.add_wpn(name, type, wa, con, av, dmg, ammo, shts, rof, rel, range, cost, category=category,
                              source=source)
    ...
    def update_sql_wpn(self, name, type, wa, con, av, dmg, ammo, shts, rof, rel, range, cost, weight, flags, options,
                       alt_munitions, description, category):
        wpn_table = self.db.table('wpn_blueprints')
        wpn_table.update_wpn(name, type, wa, con, av, dmg, ammo, shts, rof, rel, range, cost, weight, flags, options,
                             alt_munitions, description, category)
    ...
    def load_skill_bp_into_prefs(self):
        ...
        for skill in skills:
            self.prefs.set_skill_blueprint(skill.name, skill.stat, skill.category, skill.description, skill.chip,
                                           skill.diff, skill.short)
    ...
    def load_character_from_database(self, name):
        chars = self.db.table('characters')
        db_char = chars.get_by_name(name)
        c = Character()
